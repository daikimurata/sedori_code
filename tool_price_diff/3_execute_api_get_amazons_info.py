...
# ===============================================================================================================================
            ...
            # 差額計算
            dif = int(amazon_price) -int(price)
            # 手数料（FBAの固定費＋差額）
            r_dif = dif - (int(amazon_price)*0.1)
            # 利益率の計算
            br = int(r_dif/float(price))*100
            ...
            strings = str(JAN)+','+str(asin)+','+str(name)+','+str(price)+','+str(amazon_price)+','+str(dif)+','+str(int(r_dif))+','+str(br)+','+str(url)+','+str(amazon_url)+','+str(group)+','+str(rank)+'\n'
            b.write(strings)
# ===============================================================================================================================
...
# Amazonの売上ランキング
# ＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
b.write('JAN,ASIN,Item_NAME,site_PRICE,amazon_PRICE,DIF,R_DIF,BR,URL,amazonURL,Category,Ranking\n')
